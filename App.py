...
# --- Khởi tạo session_state (Để trống hoàn toàn để người dùng tự điền/tự chọn) ---
if "form_data" not in st.session_state:
    st.session_state.form_data = {
        "name": "",
        "phone": "",
        "purpose": "1. Vay mua bất động sản (Nhà ở, đất ở, căn hộ)",
        "requested_amount": 10000000,   # Giá trị khởi điểm tối thiểu để không bị lỗi số 0
        "monthly_income": 0,
        "credit_score": 700,            # Điểm tín dụng chuẩn trung bình
...
# ==========================================
    ...
    st.info(f"💡 Đang nạp dữ liệu khách hàng: **{st.session_state.form_data['name'] if st.session_state.form_data['name'] else 'Chưa đặt tên'}**")
    ...
    with col_c1:
        ...
        repayment_method = st.selectbox(
            "Phương thức trả nợ:",
            [
                "Dư nợ giảm dần (Gốc trả đều, lãi tính trên dư nợ còn lại)", 
                "Trả góp đều hàng tháng (Gốc + Lãi cố định hằng tháng)", 
                "Trả nợ tăng dần (Gốc trả tăng dần theo kỳ hạn)"
            ]
        )
    ...
    # Đối với phương thức tăng dần, áp lực trả nợ lớn nhất rơi vào tháng cuối cùng
    ...
    if is_safe:
        if st.button("💾 Lưu Hồ Sơ An Toàn Lên Hệ Thống Quản Trị", type="primary"):
            if not st.session_state.form_data["name"].strip() or not st.session_state.form_data["phone"].strip():
                st.error("Vui lòng điền đầy đủ Họ tên và Số điện thoại khách hàng ở mục Tiếp Nhận!")
            else:
