...
# --- Khởi tạo session_state lưu trữ dữ liệu form thô ---
...
st.sidebar.info("💡 Điền thông tin tại mục 'Tiếp nhận' và chuyển sang 'Công cụ tính toán' để kiểm tra điều kiện lưu trữ an toàn.")
...
if menu == "➕ Tiếp Nhận Hồ Sơ Mới":
    ...
    st.markdown("Nhập thông tin khách hàng. Hệ thống không gán sẵn giá trị để bạn tự do nhập liệu chính xác.")
    ...
    with col1:
        ...
        st.session_state.form_data["requested_amount"] = st.number_input(
            ...
            min_value=0.0, 
            value=float(st.session_state.form_data["requested_amount"]), 
            step=10000000.0, 
            format="%.0f"
        )
        ...
        st.session_state.form_data["monthly_income"] = st.number_input(
            ...
            min_value=0.0, 
            value=float(st.session_state.form_data["monthly_income"]), 
            step=5000000.0, 
            format="%.0f"
        )
    ...
    with col2:
        ...
        if st.session_state.form_data["has_collateral"]:
            ...
            st.session_state.form_data["collateral_value"] = st.number_input(
                ...
                min_value=0.0, 
                value=float(st.session_state.form_data["collateral_value"]), 
                step=50000000.0, 
                format="%.0f"
            )
        else:
            st.session_state.form_data["collateral_type"] = "Không có tài sản bảo đảm"
            st.session_state.form_data["collateral_value"] = 0.0
...
# ==========================================
    ...
    st.title("🧮 Công Cụ Tính Toán Nợ Vay & Xét Duyệt Rủi Ro")
    st.info(f"💡 Đang nạp dữ liệu từ khách hàng: **{st.session_state.form_data['name'] if st.session_state.form_data['name'] else 'Chưa nhập tên'}**")
    
    col_c1, col_c2 = st.columns([1, 1])
    
    with col_c1:
        st.subheader("⚙️ Điều chỉnh tham số vay")
        income = st.number_input("Thu nhập hàng tháng (VNĐ):", value=float(st.session_state.form_data["monthly_income"]), step=5000000.0, format="%.0f")
        loan_amount = st.number_input("Số tiền muốn vay (VNĐ):", value=float(st.session_state.form_data["requested_amount"]), step=50000000.0, format="%.0f")
        ...
        col_val = st.number_input("Giá trị định giá TSBD (VNĐ):", value=float(st.session_state.form_data["collateral_value"]), step=50000000.0, format="%.0f")
    ...
    # --- ĐÁNH GIÁ TỔNG QUAN & PHÂN QUYỀN NÚT LƯU ---
    st.subheader("🎯 Kết Luận & Quyết Định Phê Duyệt")
    ...
    if is_safe:
        st.success("✅ **HỒ SƠ ĐẠT TIÊU CHÍ AN TOÀN (RỦI RO THẤP):** Đủ điều kiện chuẩn hóa để lưu lên hệ thống quản trị của Admin.")
        
        # CHỈ CHO PHÉP BẤM NÚT LƯU KHI AN TOÀN
        if st.button("💾 Lưu Hồ Sơ An Toàn Lên Kho MySQL", type="primary"):
            if not st.session_state.form_data["name"].strip() or not st.session_state.form_data["phone"].strip():
                st.error("Vui lòng quay lại tab 'Tiếp Nhận Hồ Sơ Mới' để điền đầy đủ Họ tên và Số điện thoại khách hàng!")
            else:
        ...
        st.error("❌ **HỒ SƠ CÓ RỦI RO CAO:** Vượt quá giới hạn an toàn (DTI > 50% hoặc LTV > 70%). Hệ thống **không cho phép** lưu hồ sơ này lên kho quản trị.")
